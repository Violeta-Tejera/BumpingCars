...
_HID_REPORT_UUID = bluetooth.UUID(0x2A4D)  # HID Report
_CLIENT_CHARACTERISTIC_CONFIG = bluetooth.UUID(0x2902)
...
class GameSirController:
    def __init__(self):
        ...
        self.current_service_handle = None
    ...
    def _discover_characteristics(self, start_handle, end_handle):
        try:
            print(f"Descubriendo características para servicio: {start_handle:04x} - {end_handle:04x}")
    ...
    def _enable_notifications(self, char_handle):
        try:
            ...
            # Escribir 0x0001 en el descriptor de configuración
            self.ble.gattc_write(self.conn_handle, char_handle + 1, struct.pack('<H', 0x0001))
            print("Notificaciones habilitadas correctamente")
        except Exception as e:
        ...
        try:
            if event == _IRQ_SCAN_RESULT:
                ...
                if name and ("GAMESIR" in name.upper() or "GAMEPAD" in name.upper()):
                    ...
                    time.sleep_ms(100)  # Pequeña pausa antes de conectar
                ...
                print("╚════════════════════════════════════╝")
                time.sleep_ms(500)  # Esperar antes de descubrir servicios
                ...
                if uuid == _HID_SERVICE_UUID:
                    ...
                    time.sleep_ms(100)  # Pequeña pausa antes de descubrir características
                ...
                # Verificar si es una característica que soporta notificaciones
                if properties & 0x10:  # Propiedad de notificación
                    ...
                    time.sleep_ms(100)  # Pequeña pausa antes de habilitar notificaciones
    ...
    def _process_input_report(self, data):
        try:
            if data != self.last_report:
                ...
                # Analizar joysticks (ajustar según el formato real del GameSir)
